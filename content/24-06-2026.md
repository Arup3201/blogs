...
> .env contains environment variables like the API URL.

In this `.env` file, I have the following line,

```
VITE_API_URL=http://localhost:8080
```

This will be imported to the codebase using `import.meta.env.VITE_API_URL`.

**AuthStore**

`AuthStore` is used to store the access token and user data. Currently, I am storing the data inside `localStorage`.

> Storing sensitive data like the token inside `localStorage` is not safe. It is just the initial code to get started.
...
The constant `API_URL` will be used throughout the code to access the API URL. `AuthStore` will be used to get access to the token.
...
import { API_URL, AuthStore } from "../utils/api"
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if (data.access_token && data.user) {
            ...
            throw new Error("Response is missing access token and user data")
          }
        }
      } catch (err) {
        console.error(err)
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border p-8">
        ...
        <form className="space-y-4" onSubmit={handleLogin}>
          <div>
            <label id="email" className="block text-sm font-medium mb-1">
              Email
            </label>
            <input
              name="email"
          ...
          <div>
            <label id="password" className="block text-sm font-medium mb-1">
              Password
            </label>
            <input
              name="password"
...
export default function TasksPage() {
  ...
  async function getAllTasks(token) {
    try {
      ...
      if (response.status === 200) {
        const data = await response.json()
        setTasks(data.tasks.map((t) => transformTask(t)))
    ...
    if (editingTask) {
          ...
          setTasks((prev) =>
            prev.map((t) => {
              if (t.id === editingTask.id) {
                return { ...editingTask, ...transformTask(data.task) }
              }
      ...
      try {
        ...
        if (response.status === 201) {
          const data = await response.json()
          setTasks((prev) => [{ ...transformTask(data.task) }, ...prev])
        } else if (response.status === 401) {
          AuthStore.delete()
          navigate("/login")
        } else {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setTasks((prev) =>
          prev.map((t) => {
            if (t.id === taskId) {
              return { ...task, ...transformTask(data.task) }
            }
  ...
  const confirmDelete = async () => {
    ...
    try {
      const task = tasks.find((task) => task.id === deleteTask.id)
